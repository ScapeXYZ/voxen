...
Configure the five public variables documented in `.env.example`. `lib/voxen/config.ts` supplies the Intelligent Contract address, GenLayer RPC, EVM RPC, external chain ID, and network display name. A wallet chain ID match alone does not establish network identity. Never add secrets to public variables.
...
- `lib/voxen/config.ts`: the active deployment and RPC configuration.
