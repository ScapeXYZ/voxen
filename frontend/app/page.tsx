...
export default function Home() {
  return <main id="main" className="home">
    ...
    <section className="proof-section-new" aria-labelledby="proof-heading"><div className="shell proof-section-grid"><div><span className="eyebrow">05 / LIVE PROOF</span><h2 id="proof-heading">Inspect the system behind every decision</h2><p>Voxen writes governance state to an Intelligent Contract on {ACTIVE_NETWORK.name}. Live proof connects the interface to the network facts behind it.</p><Link href="/live-proof" className="button button-light">View live proof <ArrowUpRight size={16} /></Link></div><div className="proof-panel proof-reference"><span className="eyebrow">ACTIVE DEPLOYMENT</span><dl><div><dt>Network</dt><dd>{ACTIVE_NETWORK.name}</dd></div><div><dt>Chain</dt><dd>{ACTIVE_NETWORK.chainId}</dd></div><div><dt>Contract</dt><dd><code>{ACTIVE_NETWORK.contract.slice(0, 10)}…{ACTIVE_NETWORK.contract.slice(-8)}</code></dd></div></dl><p className="connected-governance-state"><i className="live-dot" /> <span>Connected governance state</span></p></div></div></section>
    <section className="home-section shell live-preview reveal" aria-labelledby="live-heading">
      <header className="landing-proposals-header">
        <div className="landing-proposals-intro">
          <p className="eyebrow"><i className="live-dot" /> LIVE NETWORK</p>
          <h2 id="live-heading">See governance in motion</h2>
          <p>Live proposal data from the active Studio Next deployment. Browse without connecting a wallet.</p>
          <p className="collection-note">Newest first · Status counts apply to loaded live proposals.</p>
        </div>
        <Link href="/explore" className="landing-proposals-action">View all proposals <ArrowRight size={16} /></Link>
      </header>
      <LiveExplore />
    </section>
